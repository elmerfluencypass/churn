...
def run():
    ...
    st.write("📄 Colunas cadastro:", cadastro.columns.tolist())
    st.write("📄 Colunas invoices:", invoices.columns.tolist())
    st.write("📄 Colunas subscrição:", subs.columns.tolist())
    ...
    if 'data_inicio_curso' in cadastro.columns:
        cadastro['data_inicio_curso'] = pd.to_datetime(cadastro['data_inicio_curso'], errors='coerce')
    if 'ultima_data_pagamento' in cadastro.columns:
    ...
    if 'ultima_data_pagamento' in cadastro.columns:
        ...
        st.error("Coluna 'ultima_data_pagamento' ausente em cadastro_clientes.")
        return
    ...
    df['mes_desistencia'] = df['ultima_data_pagamento'].dt.month_name() if 'ultima_data_pagamento' in df else "Desconhecido"
    df['periodo_curso'] = ((df['ultima_data_pagamento'] - df['data_inicio_curso']) / pd.Timedelta(days=30)).fillna(0).astype(int)

    st.metric("🧑‍🎓 Alunos Desistentes", df['user_id'].nunique() if 'user_id' in df.columns else len(df))
    st.metric("💰 Receita Perdida Total (R$)", f"{df['receita_perdida'].sum():,.2f}")

    alunos_unicos = sorted(df['nome'].dropna().unique()) if 'nome' in df.columns else []
